...
def handle_pretest_v_operational(d_df, f_df, file_name):
    ...
    t_df['GROUP_NAME'] = d_df['GROUP_NAME']

    evaluate_times(t_df)
    stats.get_stats(t_df, 0,'GROUP_NAME', target_index=True)
    ...
    compare = stats.get_stats(d_df, 'Score', 'GROUP_NAME')
    s = (processed.iloc[:,0] - compare['Score']).sum()
    return processed, items

def evaluate_times(df):
    stats_dfs = []
    anova_dfs = []
    for i in range(df.shape[1]-1):
        group_df = stats.get_group_df(df, target_index=i, group_name='GROUP_NAME')
        stats_df = stats.get_group_stats(group_df)
...
def item_level_analysis(df, group_var='GROUP_NAME'):
    ...
    for i in range(df.shape[1] - 3):
        ...
        #testing df delete me
        df.to_csv("check_key.csv")
        ##  DAVIS:
        #   vs US
        a = stats.chisq_2_2(correct[DAVIS], incorrect[DAVIS], correct[US], incorrect[US])
        p = df.sum(axis = 0)
        # vs US+WESTERN
        b = stats.chisq_2_2(correct[DAVIS], incorrect[DAVIS], correct[US] + correct[WESTERN],
                      incorrect[US] + incorrect[WESTERN])
        # vs ALL
        c = stats.chisq_2_2(correct[DAVIS], incorrect[DAVIS], correct[US] + correct[WESTERN] + correct[FOREIGN],
                      incorrect[US] + incorrect[WESTERN] + incorrect[FOREIGN])

        ##WESTERN:
        d = stats.chisq_2_2(correct[WESTERN], incorrect[WESTERN], correct[US], incorrect[US])
        # vs US+WESTERN
        e = stats.chisq_2_2(correct[WESTERN], incorrect[WESTERN], correct[US] + correct[DAVIS],
                      incorrect[US] + incorrect[WESTERN])
        # vs ALL
        f = stats.chisq_2_2(correct[WESTERN], incorrect[WESTERN], correct[US] + correct[DAVIS] + correct[FOREIGN],
...
def process_dfs(op_ans, op_time, pre_ans, pre_time):
    opa_r = stats.get_stats(op_ans, 'SCORE', 'GROUP_NAME')
    opt_r = stats.get_stats(op_time, 'TIME', 'GROUP_NAME')
    pra_r = stats.get_stats(pre_ans, 'SCORE', 'GROUP_NAME')
    prt_r = stats.get_stats(pre_time, 'TIME', 'GROUP_NAME')
